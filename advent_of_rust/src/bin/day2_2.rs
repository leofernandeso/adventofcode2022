...
static LOSES_AGAINST: phf::Map<char, char> = phf_map! {
    'C' => 'A', 'A' => 'B', 'B' => 'C',
};
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    for line in lines {
        if let Ok(line) = line {
            ...
            let player_strategy: char = parse_split_into_choice_character(&mut split_line);
            let player_choice: char = get_player_action(player_strategy, opponent_choice);
...
fn get_player_action(player_strategy: char, opponent_choice: char) -> char {
    match player_strategy {
        'X' => WINS_AGAINST.get(&opponent_choice).unwrap().clone(),     // should lose
        'Z' => LOSES_AGAINST.get(&opponent_choice).unwrap().clone(),    // should win
        _ => opponent_choice  // should draw
    }
}
