...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let total_of_top_three_carbs = find_total_of_top_three_carbs(lines)?;
    println!("{}", total_of_top_three_carbs);

    Ok(())
}

fn find_total_of_top_three_carbs(input_file_lines: io::Lines<BufReader<File>>) -> Result<u64, Box<dyn Error>> {
    let mut carbs_vec = Vec::new();
    ...
    carbs_vec.sort();
    Ok(carbs_vec[carbs_vec.len() - 3..].iter().sum())
